...
logger = LoggingHandler(f"{prop().get_result_log_file_path()}FDSGradientBoostedTreeClassifier", "a", "DEBUG")


class FDSGradientBoostedTreeClassifier(FDSModel):

    model: GBTClassifier
    train_validation_split: TrainValidationSplit

    def __init__(self, data: SparkDataObject):
        super(FDSGradientBoostedTreeClassifier, self).__init__(logger.get_log(), data)
